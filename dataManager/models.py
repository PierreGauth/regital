...
class Transaction(models.Model): # relevé de compte
	...
	date_transaction = models.DateField('date', null=False)
	...
	def __unicode__(self):
		return '{0.date_transaction!s} - {0.libelle}: {0.montant!s}'.format(self)

class TransactionSoiree(Transaction):
	budget_soiree = models.OneToOneField('BudgetSoiree')
...
class BudgetSoiree(models.Model):
	...
	r_debit_total_reg = models.IntegerField(null=True, blank=True) 	# ???
	r_credit_total_reg = models.IntegerField(null=True, blank=True)	# ???
	...
	def __unicode__(self):
		return 'Le {0.soiree.date_soiree}: Dépenses={0.total_depenses_reg!s} | Recettes={0.total_recettes_reg!s}'.format(self)
...
class Recapitulatif(models.Model):
	...
	date_recapitulatif = models.DateField() # date de réalisation du récapitulatif couvrant la période précédente
...
#	transaction = models.ForeignKey(Transaction, on_delete=models.CASCADE)
	class Meta:
		unique_together=(('date_recapitulatif', 'frequence',),)

	def __unicode__(self):
		return 'Bilan {0.frequence} du {0.date_recapitulatif!s}: Dépenses={0.total_depenses_reg!s}d | Recettes={0.total_recettes_reg!s}d'.format(self)
...
class Soiree(models.Model):
	date_soiree = models.DateField(unique=True)
	libelle_date_soiree_reg = models.CharField(max_length=64)
	budget_soiree = models.OneToOneField(BudgetSoiree, null=True, blank=True)
	ligne_src = models.IntegerField(null=False)

	def __unicode__(self):
		return 'Soirée du {0.date_soiree} ({0.libelle_date_soiree_reg})'.format(self)
...
class Representation(models.Model):
	...
	def __unicode__(self):
		return 'La pièce {0.piece.titre!r} jouée le {0.soiree.date_soiree!s} ({0.position!s})'.format(self)
...
class Animation(models.Model):
	...
	type_animation = models.IntegerField(choices=T_ANIMATION)
	...
	class Meta:
		unique_together=(('type_animation', 'soiree', 'position'),)

	def __unicode__(self):
		return 'Un {0.type_animation} est donné le {0.soiree.date_soiree!s} ({0.position!s})'.format(self)
